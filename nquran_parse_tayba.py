...
conn = sqlite3.connect('E:/Qeraat/QeraatFasrhTools_Data/qeraat_data_simple.db')
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS quran_data_tayba (
        id INTEGER,
        sora INTEGER,
        aya INTEGER,
        subject TEXT,
            ...
            for selectquran_tag in selectquran_tags:
                ...
                for j, quran_page in enumerate(quran_pages, start=1):
                    ...
                    for i, content_line in enumerate(content_lines):
                        content_line_pure = araby.strip_diacritics(content_line) 
                        for k, rawy in enumerate(all_rowa):
                            if content_line_pure.endswith(rawy):
                                last_match_index = i
