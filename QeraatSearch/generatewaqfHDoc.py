...
SELECT
    qd.page_number2,
    SUBSTR(qd.reading, INSTR(qd.reading, 'وقف ب'))  AS waqf,
    GROUP_CONCAT('' || qd.aya || ' ـ ' || qd.sub_subject || '') AS aya_and_sub_subject
FROM
    quran_data qd
WHERE qd.qarees LIKE '%هشام%'  and qd.reading like '%وقف ب%'
GROUP BY
    qd.page_number2, SUBSTR(qd.reading, INSTR(qd.reading, 'وقف ب'))
ORDER BY
    qd.page_number2, SUBSTR(qd.reading, INSTR(qd.reading, 'وقف ب'))
"""
