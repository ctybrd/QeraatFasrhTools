...
def create_xfdf(input_pdf, output_xfdf, db_file):
    ...
    for row in data:
        ...
        annot = f'''
        <line start="{x_start},{y_start}" end="{x_end},{y_end}" title="Me" creationdate="{creation_date}" subject="Line" page="{page_number}" date="{creation_date}" flags="print" name="{annot_name}" rect="{x_start},{y_start - 0.5},{x_end},{y_start + 0.5}" color="{color}"/>
        '''
        annots.append(annot)
