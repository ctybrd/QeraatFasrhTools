...
FROM (
    SELECT 
        ...
        CASE 
            ...
            ELSE ' )جميعا(' 
        END as csub_subject,
        replace(reading,'، مع ترك الوقف بهاء السكت','') as creading,
        ifnull(readingresult, '') as readingresult,
        printf('%03d', sora) || printf('%03d', aya) || printf('%03d', id) as ayas 
    FROM quran_data 
    WHERE 
        r1_1 is not null and IFNULL(r5_2, 0) = 0 
        and sub_sno = 1
        and reading<>'بصلة ميم الجمع وصلا بخلف.'
        and reading <>'بترقيق اللام.'
    ORDER BY page_number1, aya, id
) AS subquery
GROUP BY page_number1, creading, readingresult
ORDER BY page_number1, ayas;
...
for page_number1 in range(1, 605):  # Assuming you have 522 pages
    image_path = f'E:/pagesqalonM/{page_number1}.png'
    ...
    if os.path.exists(image_path):
        ...
        run.add_picture(image_path, width=Cm(17))  
        if page_number1 % 2 == 0:
        ...
        matching_rows = [row for row in rows if row[0] == page_number1]
        ...
        # Add a page break after each page_number1 change
        doc.add_page_break()

# Save the Word document
doc.save('e:/pageshamza/Qaloon.docx')
